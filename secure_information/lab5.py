...
def diffie_hellman_key_exchange():
    ...
    if mode == 1:
        ...
        p = generate_prime(10 ** 8, 10 ** 9)  # Большие числа порядка 10^9

        # Поиск примитивного корня (генератора)
...
def elgamal_generate_keys(mode: int = 1) -> Tuple[Tuple[int, int, int], Tuple[int, int]]:
    ...
    if mode == 1:
        ...
        # Генерация ключей
        D1 = random.randint(2, p - 2)  # Секретный ключ
        C1 = mod_exp(g, D1, p)  # Открытый ключ
...
def elgamal_encrypt_file(public_key: Tuple[int, int, int], input_file: str, output_file: str):
    ...
    # Преобразование в числа (блоками)
    block_size = (p.bit_length() - 1) // 8  # Размер блока в байтах
    encrypted_blocks = []
    ...
    for i in range(0, len(data), block_size):
        ...
        if m >= p:
            ...
            for j in range(0, len(block), sub_block_size):
                ...
                m_sub = int.from_bytes(sub_block, 'big')
                encrypted_blocks.extend(elgamal_encrypt_block(p, g, C1, m_sub))
        else:
            encrypted_blocks.extend(elgamal_encrypt_block(p, g, C1, m))
    ...
    with open(output_file, 'wb') as f:
        ...
        # Записываем каждый блок (a, b)
        for a, b in encrypted_blocks:
            f.write(a.to_bytes((p.bit_length() + 7) // 8, 'big'))
            f.write(b.to_bytes((p.bit_length() + 7) // 8, 'big'))


def elgamal_encrypt_block(p: int, g: int, C1: int, m: int) -> List[Tuple[int, int]]:
    """Шифрование одного блока данных"""
    k = random.randint(2, p - 2)  # Случайное число k
    a = mod_exp(g, k, p)  # a = g^k mod p
    b = (m * mod_exp(C1, k, p)) % p  # b = m * C1^k mod p
...
def elgamal_decrypt_file(private_key: Tuple[int, int], input_file: str, output_file: str):
    ...
    with open(input_file, 'rb') as f:
        ...
        for _ in range(num_blocks):
            a = int.from_bytes(f.read(block_size), 'big')
            b = int.from_bytes(f.read(block_size), 'big')
    ...
    for a, b in encrypted_blocks:
        ...
        s = mod_exp(a, D1, p)
        # Находим обратный элемент s_inv
        _, s_inv, _ = extended_euclid(s, p)
        s_inv %= p

        # Восстанавливаем сообщение: m = b * s_inv mod p
        m = (b * s_inv) % p

        # Преобразуем число обратно в байты
        block_size = (m.bit_length() + 7) // 8
...
def main():
    ...
    print("6 - Шифр Эль-Гамаля (шифрование/дешифрование файлов)")
