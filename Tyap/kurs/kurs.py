...
# --------- Логика построения ДКА ---------

def build_suffix_automaton(alphabet, suffix):
    """
    Автомат, отслеживающий, какой префикс suffix совпадает с текущим суффиксом входа.
    Состояния: 0..len(suffix), состояние i означает, что совпадает префикс длины i.
    """
    n = len(suffix)

    # массив границ (как в КМП)
    border = [0] * n
    k = 0
    for i in range(1, n):
...
def build_dfa(alphabet_str, suffix, count_symbol, k):
    ...
    # внутренние состояния как пары (i, r)
    internal_states = []
    for i in range(len(suffix) + 1):
        for r in range(k):
            internal_states.append((i, r))

    # отображение внутренних состояний в имена q0, q1, ...
    state_to_name = {}
    name_to_state = {}
...
class DFAApp:
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Запись результатов в файл", command=self.save_results_to_file)
    ...
    def show_author(self):
        messagebox.showinfo(
            ...
            "Группа: ХХХ-ХХ\n"
            "Тема: Построение ДКА по описанию языка.\n"
    ...
    def show_help(self):
        help_text = (
            ...
            "Подцепочка: строка над алфавитом, которой должны заканчиваться все цепочки.\n"
            "Символ кратности: один символ из алфавита.\n"
            "Кратность: целое число > 0; число вхождений выбранного символа делится на него.\n\n"
            "Конечное состояние — это состояние автомата,\n"
            "в которых он останавливается после чтения всей строки и принимает её."
        )
    ...
    # --- вкладка "Автомат": параметры, таблица, граф ---
    ...
    def create_automaton_tab(self):
        ...
        tk.Label(top_frame, text="Алфавит (через запятую):").grid(row=0, column=0, sticky="w")
        self.entry_alphabet = tk.Entry(top_frame, width=35)
        self.entry_alphabet.insert(0, "0,1")
        self.entry_alphabet.grid(row=0, column=1, sticky="we", padx=2)

        tk.Label(top_frame, text="Конечная подцепочка:").grid(row=1, column=0, sticky="w")
        self.entry_suffix = tk.Entry(top_frame, width=35)
        self.entry_suffix.insert(0, "01")
        ...
        self.btn_build = tk.Button(top_frame, text="Построить автомат", command=self.build_automaton)
        self.btn_build.grid(row=4, column=0, columnspan=2, pady=5)

        top_frame.grid_columnconfigure(1, weight=1)

        # Нижняя часть: слева таблица переходов, справа граф
        bottom_frame = tk.Frame(frame)
        bottom_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # Таблица переходов (Treeview)
    ...
    # --- вкладка "Проверка": проверка цепочек ---
    ...
    def create_check_tab(self):
        ...
        tk.Label(top_frame, text="Цепочка для проверки:").grid(row=0, column=0, sticky="w")
        self.entry_word = tk.Entry(top_frame, width=40)
        self.entry_word.insert(0, "0101")
        self.entry_word.grid(row=0, column=1, sticky="we", padx=2)

        self.btn_check = tk.Button(top_frame, text="Проверить цепочку", command=self.check_word)
        self.btn_check.grid(row=1, column=0, pady=5)

        self.btn_step = tk.Button(top_frame, text="Следующий шаг", command=self.next_step)
    ...
    def update_table(self):
        """Обновить таблицу переходов: строки — q0, q1, ..., столбцы — символы алфавита."""
    ...
    def format_state(self, state):
        """Вернуть короткое имя состояния (q0, q1, ...)."""
        if not self.dfa:
            return str(state)
        return self.dfa["state_to_name"].get(state, str(state))

    def draw_graph(self):
        """Простая визуализация графа автомата на Canvas (с короткими именами состояний)."""
        ...
        r_node = 18  # радиус вершины

        # рёбра с явными стрелками
        for (q, a), nxt in self.dfa["trans"].items():
            ...
            # сдвигаем начало и конец чуть наружу, чтобы линия не пересекала центры
            ...
            # подпись символа примерно посередине ребра
            mx = (sx + ex) / 2
            my = (sy + ey) / 2
            self.canvas.create_text(mx, my, text=a, fill="blue")
    ...
    def check_word(self):
        if not self.dfa:
            messagebox.showwarning("Нет автомата", "Сначала постройте автомат на вкладке 'Автомат'.")
            return
        ...
        try:
            ...
            log_lines.append(f"Начальное состояние: {self.format_state(self.dfa['start'])}")
            for s in steps:
                log_lines.append(
            ...
            if final_state in self.dfa["accepts"]:
                ...
                log_lines.append("Результат: цепочка НЕ принадлежит языку (финальное состояние не является конечным).")
